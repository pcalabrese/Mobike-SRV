...
public class RouteServicesImpl implements RouteServices {
	...
	/**
	 * Method to add a Route to persistence and create gpx file in the server
	 * filesystem
	 * 
	 * @param json
	 *            JSON file containing the Route Details formatted in this way:
	 *            Route [id=" + id + ", name=" + name + ",
	 *            creatorEmail=" + creatorEmail + ", duration=" + duration + ",
	 *            length=" + length +", uploadDate=" + uploadDate +",
	...
	public Response createRoute(String wrappingJson) {
		...
		if (wrappingJson != null) {
			...
			try {
				exists = userRepo.userExists(route.getOwner().getId(), route
			...
			if (exists) {
				...
				try {
					...
					throw new UncheckedFilesystemException(
							"Error saving file to filesystem" + e.getMessage(),
							e);
				}
	 ...
	 * 
	 * @param id
	 *            Long ID of the requested Route
	 * @return Response OK: json formatted Route, FAIL: Error404
	 * @see Route Route: Model Class
	 * @see RouteMySql RouteMySql: SQL DAO Class
	 * @see GpxIO GpxIO: class to read and write from and to file in the server
	 *      filesystem
	 * @see Views.ItineraryDetailView Views.ItineraryDetailView: View for
	...
	public Response getRoute(@PathParam("Id") Long id) {
		...
		if (route != null) {
			...
			objectMapper.setConfig(objectMapper.getSerializationConfig()
					.withView(Views.ItineraryDetailView.class));
			objectMapper.configure(MapperFeature.DEFAULT_VIEW_INCLUSION, false);

			// starting JSON serialization
			try {
				json = objectMapper.writerWithView(
						Views.ItineraryDetailView.class).writeValueAsString(
						route);
	 ...
	 * @see Views.ItineraryGeneralView Views.ItineraryGeneralView: General View
	 *      to visualize a list of Routes
	 * @version 3.1
	 * 
	 */
	@GET
	...
	@GET
	@Path("/retrievefiltered")
	@Produces(MediaType.APPLICATION_JSON)
	@Override
	public Response retrieveAllFilteredRoutes(
			...
			@QueryParam("minDuration") int minDuration,
		...
		try {
			allRoutes = routeRep.searchRouteByCriteria(startLocation, endLocation, minLength, maxLength, minDuration, maxDuration, type);
		} catch (Exception e) {
			throw new UncheckedPersistenceException(
					"Error accessing route database", e);
		}
